...
__device__ type manh_dist(position start, position end) {
    return abs(int64_t(end.x) - int64_t(start.x)) + abs(int64_t(end.y) - int64_t(start.y));
}
