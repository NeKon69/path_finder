    ...
    auto find_path() {
        auto run_algo = [&](auto&&... extra_args) {
            return launch_queue_pf(
                array.get(), q1.get(), q2.get(), q1_cnt.get(), q2_cnt.get(),
                width, height, start, end, flag_finished.get(),
        ...
        auto [path, time_spent] = [&]() {
            if (MODE == mode::gpu) {
                raw::cuda_wrappers::buffer<position> d_path(
                    width + height * sizeof(position), stream);
